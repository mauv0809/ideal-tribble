...
func SlackEventsHandler(store club.ClubStore, notifier notifier.Notifier, matchmakingService matchmaking.MatchmakingService, cfg config.Config) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if eventPayload.Type == "event_callback" {
			...
			if eventPayload.Event.Channel != cfg.Slack.ChannelID {
			...
			if eventPayload.Event.Type == "reaction_added" {
				...
				if err := handleReactionAdded(eventPayload.Event.User, eventPayload.Event.Reaction, eventPayload.Event.Item.Timestamp, eventPayload.Event.Item.Channel, store, matchmakingService); err != nil {
			...
			if eventPayload.Event.Type == "reaction_removed" {
				...
				if err := handleReactionRemoved(eventPayload.Event.User, eventPayload.Event.Reaction, eventPayload.Event.Item.Timestamp, eventPayload.Event.Item.Channel, store, matchmakingService); err != nil {
...
func handleReactionAdded(slackUserID, reaction, messageTimestamp, channelID string, store club.ClubStore, matchmakingService matchmaking.MatchmakingService) error {
	...
	log.Info("Added player availability from reaction", "player", foundPlayer.Name, "day", day, "requestID", requestID)
	return nil
}

// handleReactionRemoved processes emoji reaction removals for availability collection
func handleReactionRemoved(slackUserID, reaction, messageTimestamp, channelID string, store club.ClubStore, matchmakingService matchmaking.MatchmakingService) error {
	// Check if this is a reaction to an active match request message
	requestID, isActiveRequest, err := matchmakingService.IsActiveMatchRequestMessage(messageTimestamp)
	if err != nil {
		return fmt.Errorf("failed to check if message is active match request: %w", err)
